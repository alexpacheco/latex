...
\pdsetup{
  palette=lehigh,
  logohook=tl,
  logopos={0.01\slidewidth,0.998\slideheight},
  logocmd={\includegraphics[trim=0 -1 0 0,clip,height=.07\slidewidth,keepaspectratio=true]{njit.eps}},
  lf={\color{black}Creating Documents with \LaTeX{}},
  %lf={\color{black}\sectiontitle},
  cf={\color{black}Research Computing Seminars},
  theslide={\color{black}\arabic{slide}~/~\pageref*{lastslide}}
}
 
\definecolor{pdcolor2}{RGB}{192,192,192}%njitgray
\definecolor{pdcolor3}{RGB}{204,0,0}%njitred
...
\begin{wideslide}[bm={Theorems, Lemmas and Corollaries},method=file]{Theorems, Lemmas and Corollaries}
  \begin{itemize}
      \item Mathematical documents include elements that require special formatting and numbering such as theorems, definitions, propositions, remarks, corollaries, lemmas and so on.
      \item Make sure you have the amsthm package enabled: \lstinline|\usepackage{amsthm}|
     \item In the preamble, add \lstinline|\newtheorem{name}{Printed output}|
     \item The first argument is the name of the environment that is defined, the second argument is the word that will be printed, in boldface font, at the beginning of the environment. 
     \item For example: add \lstinline|\newtheorem{theorem}{theorem}| to preamble and the following within the document.
     \begin{LTXexample}[numbers=none,pos=b]
\begin{theorem}
...
\begin{wideslide}[toc=,bm={ },method=direct]{Theorems, Lemmas and Corollaries}
  \begin{itemize}
     \item You can add a name within brackets in the environment opening command
\begin{LTXexample}[numbers=none,pos=b]
\begin{theorem}[Pythagorean theorem]
\label{pythagorean}
This is a theorem about right triangles summarised by 
equation 
\[ x^2 + y^2 = z^2 \]
\end{theorem}
\end{LTXexample}
     \item The \lstinline|\newtheorem| command accepts different options to change the numbering style
     \item \lstinline|\newtheorem{theorem}{Theorem}[section]|: restart the theorem counter for each section.
     \item \lstinline|\newtheorem{lemma}[theorem]{Lemma}|: Create a new environment lemma that uses the same counter as theorem.
     \item \lstinline|\newtheorem{corollary}{Corollary}[theorem]|: Create a new environment corollary such that the counter is reset every time a new theorem environment is used.
     \item \lstinline|\newtheorem*{remark}{remark}|: Create an unnumbered environment for remarks and comments.
  \end{itemize}
\end{wideslide}

\begin{wideslide}[bm={Proofs},method=direct]{Proofs}
\vspace{-0.5cm}
\begin{LTXexample}[numbers=none,pos=b]
\begin{corollary}
...
\begin{wideslide}[toc=,bm={},method=direct]{Proofs}
  \begin{itemize}
  \item Proofs are the core of mathematical papers and books and is customary to keep them visually apart from the normal text in the document. 
\begin{LTXexample}[numbers=none,pos=b]
  \begin{lemma}
...
\end{proof}
\end{LTXexample}
  \item A special symbol is used to mark the end of the proof. 
  \item To change the symbol to say a blacksquare, add \lstinline|\renewcommand\qedsymbol{$\blacksquare$}| to the preamble
  \end{itemize}
\end{wideslide}
